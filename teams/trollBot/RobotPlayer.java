// Robot submitted for scrimmaging that auto-resigns in the second-to-last round
// but will allow us to try new strategies while giving the impression that we're
// losing every single time (hence the package name :P)
...
public class RobotPlayer {
	...
	private static int matchDuration;
	...
	public static void run(RobotController myRC) {
		...
		if(rc.getType() == RobotType.HQ) {
			matchDuration = rc.getRoundLimit();
			swarmRound = (int) ((9.0 / 10.0) * matchDuration);
		}
			...
			try {
				...
				case HQ:
					...
					if ((friendlyRobots.length - enemyRobots.length >= 300
							...
							|| (roundNum > swarmRound && (currentNumFriendlySoldiers > 30
