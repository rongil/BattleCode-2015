...
public class RobotPlayer {
		...
		while (true) {
			...
			try {
				...
				case BEAVER:
					...
					} else if (rc.readBroadcast(NUM_FRIENDLY_AEROSPACELAB_CHANNEL) < 5) {
				...
				case HQ:
					...
					} else if(beaverCount < 3 && rc.readBroadcast(NUM_FRIENDLY_LAUNCHERS_CHANNEL) > 3) {
						if (createUnit(RobotType.BEAVER, false)) {
							rc.broadcast(NUM_FRIENDLY_BEAVERS_CHANNEL, beaverCount + 1);
						}
					}
				...
				case LAUNCHER:
					...
					if (bestTarget != null) {
						...
						} else if (friendlyTowers.length > 0) {
	...
	private static void broadcastSwarmConditions() throws GameActionException {
		...
		if (roundNum > swarmRound || launcherCount > 5) {
